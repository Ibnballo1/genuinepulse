...
import { db } from "@/db";
import { users, businesses, reviewRequests, feedback, subscriptions, messageLogs } from "@/db/schema";
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const [
    ...
    db
      ...
      .from(feedback)
      .where(
        sql`type = 'public_review' AND submitted_at >= ${thirtyDaysAgo}`
      ),
  ...
  const planRevenue = planBreakdown
    ...
    .reduce(
      (acc, row) => {
        const price = row.plan === "enterprise" ? 299 : row.plan === "pro" ? 149 : 49;
