...
// FIX: Date objects → .toISOString() before sql`` interpolation
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  // ✅ FIX: ISO string for sql`` templates
  ...
  const [
    ...
    // ✅ Use Drizzle ORM gte() — handles Date just fine
    db
      .select({ count: count() })
      .from(reviewRequests)
    ...
    db.execute(sql`
      ...
      FROM feedback
      WHERE type = 'public_review'
        AND submitted_at >= ${thirtyDaysAgoIso}::timestamptz
    `),
    ...
    db.execute(sql`
      SELECT
        ...
        COUNT(DISTINCT f.id) FILTER (WHERE f.type = 'public_review')::int AS reviews,
        ROUND(AVG(f.rating)::numeric, 1) AS avg_rating
      FROM businesses b
      LEFT JOIN review_requests rr
        ON rr.business_id = b.id
        AND rr.created_at >= ${thirtyDaysAgoIso}::timestamptz
  ...
  return NextResponse.json({
    ...
    data: {
      summary: {
        ...
        totalReviews30d: Number((totalReviews[0] as any)?.total ?? 0),
