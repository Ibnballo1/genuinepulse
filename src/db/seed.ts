...
async function seed() {
  ...
  await db
    ...
    .values(messageTemplateData.map((t) => ({ id: randomUUID(), ...t })));
  ...
  for (const r of requestsData) {
    ...
    if (r.status === "clicked" && customer.lastRating) {
      ...
      await db.insert(feedback).values({
        ...
        reviewPlatformUrl: isPublic
          ? "https://g.page/r/donovan-auto-group/review"
          : undefined,
        message: isPublic
          ? undefined
          : "The wait time was longer than expected and nobody came to greet me.",
  ...
  const historicalRatings = [
    5, 5, 5, 4, 5, 4, 5, 3, 5, 5, 4, 5, 5, 2, 5, 4, 5, 5, 5, 4, 5, 5, 1, 5, 5,
    4, 5, 5, 5, 4,
  ];
